...
func _load_runtime(path: String) -> void:
	...
	info_label.text = "%s\nBPM %.1f  |  seed %s  |  hero %s\nSpace play/pause · R restart · [ ] seek" % [
		title,
		bpm,
		str(runtime.get("seed", 0)),
		str(runtime.get("hero_scene", "valleytainment_logo")),
	]
...
func _process(delta: float) -> void:
	if not playing:
		if audio_bus.has_method("is_live") and audio_bus.is_live():
			...
			_drive_features({
				...
				"snare_energy": float(live.get("mid", 0.0)) * 0.5,
	...
	_update_section()
	_drive_features(_features_at(show_time))
	section_label.text = "%s  ·  t=%.1fs  ·  I=%.2f" % [
		current_section,
		show_time,
		float(_features_at(show_time).get("intensity", 0.0)),
	]
